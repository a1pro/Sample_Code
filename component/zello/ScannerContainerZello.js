...
import Scanner_Zello from './Scanner_Zello';
...
class ScannerContainerZello extends Component {
        ...
        then((response) => {
          ...
          if (data.length == 0) {
           ...
           // alert('Product not found. Please scan again')
            ...
            this.goToUpdatePage('UpdateDetailsContainer', Product_Num);
            this.setState({ Scan_Start: true });
          }
        }).catch((Error) => {
          this.setState({ ShowLoader: true, Scan_Start: false })
...
export default connect(mapToStateProps, { CreateList, ScannerAddtoList })(ScannerContainerZello)
